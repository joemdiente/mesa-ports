...
int spi_initialize(const char* dev)
{
	/* Initialize Variables */
	uint8_t ret = 0;
    dev_spi = dev;
            ...
            uint32_t            *value)
{
    ...
    // vtss_port_no_t port_no = ctx->port_no;
    ...
    //printf("spi_write_rbt: %s", rpi_spi_buffer);
    ...
    //printf("spi32_wr port_no = 0x%X, mmd = 0x%X, addr = 0x%X, value = 0x%X", port_no, mmd, addr, *value);
    if ((mmd == 0x1e) && (addr == 0x9002 || addr == 0x9202)) 
    {
                ...
                uint32_t            *value)
{
    ...
    // printf("%s: %s", __func__, rpi_spi_buffer);
    ...
    ////printf("spi32_rd port_no = 0x%X, mmd = 0x%X, addr = 0x%X, value = 0x%X", port_no, mmd, addr, *value);
    if ((mmd == 0x1e) && (addr == 0x9002 || addr == 0x9202))
    {
